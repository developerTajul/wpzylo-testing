...
if ( ! function_exists( 'zylo_check_breadcrumb' ) ) :
    add_action( 'zylo_before_main_content', 'zylo_check_breadcrumb' );

    function zylo_check_breadcrumb() {
        $default_breadcrumb = '1';
        if ( class_exists( 'Redux' ) ) {
            $opt_name           = 'zylo_options';
            $default_breadcrumb = Redux::get_option( $opt_name, 'choose_default_breadcrumb', '1' );
        }

        $breadcrumb_style = get_post_meta( get_the_ID(), 'zylo_choice_breadcrumb_style', true );

        if ( 'breadcrumb-style-1' === $breadcrumb_style || '1' === $default_breadcrumb ) {
            zylo_breadcrumb_style_1();
        }
    }
endif;

if ( ! function_exists( 'zylo_breadcrumb_style_1' ) ) :
    function zylo_breadcrumb_style_1() {
        $hide_breadcrumb = get_post_meta( get_the_ID(), 'zylo_invisible_breadcrumb', true );
        if ( $hide_breadcrumb ) {
            return;
        }
        ...
        $hide_bg_img              = get_post_meta( get_the_ID(), 'hide_breadcrumb_bg_img', true );

        if ( empty( $hide_bg_img ) ) {
            if ( $breadcrumb_img_from_page ) {
                $bg_img = 'background-image: url(' . esc_url( $breadcrumb_img_from_page ) . ');';
            } elseif ( class_exists( 'Redux' ) ) {
                $opt_name         = 'zylo_options';
                $breadcrumb_bg    = Redux::get_option( $opt_name, 'breadcrumb_bg_img', array( 'url' => '' ) );
                if ( ! empty( $breadcrumb_bg['url'] ) ) {
                    $bg_img = 'background-image: url(' . esc_url( $breadcrumb_bg['url'] ) . ');';
                }
            }
        }

        $blog_title         = esc_html__( 'Blog', 'zylo' );
        $blog_details_title = esc_html__( 'Blog Details', 'zylo' );
        if ( class_exists( 'Redux' ) ) {
            $opt_name           = 'zylo_options';
            $blog_title         = Redux::get_option( $opt_name, 'breadcrumb_blog_title', $blog_title );
            $blog_details_title = Redux::get_option( $opt_name, 'breadcrumb_blog_title_details', $blog_details_title );
        }

        $page_title = ( is_single() && 'post' === get_post_type() ) ? $blog_details_title : zylo_page_title();
        $title_tag  = ( is_single() && 'post' === get_post_type() ) ? 'h2' : 'h3';
        ?>
        <div class="page-breadcrumb-area page-bg pt-240 pb-160 pt-xs-165 pb-xs-100 pt-sm-165 pb-sm-100" style="<?php echo esc_attr( $bg_img ); ?>">
...
if ( ! function_exists( 'zylo_page_title' ) ) :
    function zylo_page_title() {
        if ( is_home() ) {
            ...
            return esc_html__( 'Search Results', 'zylo' );
        } elseif ( is_404() ) {
            return esc_html__( '404 - Not Found', 'zylo' );
        } else {
            return get_the_title();
        }
    }
...
if ( ! function_exists( 'zylo_breadcrumbs' ) ) :
    function zylo_breadcrumbs() {
        ...
        $home = esc_html__( 'Home', 'zylo' );
        if ( class_exists( 'Redux' ) ) {
            $opt_name = 'zylo_options';
            $home     = Redux::get_option( $opt_name, 'breadcrumb_home', $home );
        }
        ...
        echo '<li><a href="' . $home_link . '">' . esc_html( $home ) . '</a></li>';
        ...
        if ( is_category() ) {
            ...
            echo '<li class="active">' . esc_html__( 'Search results for', 'zylo' ) . ' "' . get_search_query() . '"</li>';
        } elseif ( is_day() ) {
            ...
            if ( 'post' !== get_post_type() ) {
                ...
                echo '<li><a href="' . esc_url( $home_link . '/?post_type=' . $slug ) . '">' . esc_html( $post_type->labels->singular_name ) . '</a></li>';
            ...
            echo '<li class="active">' . esc_html__( 'Page Not Found', 'zylo' ) . '</li>';
        }

        if ( get_query_var( 'paged' ) ) {
            echo '<li class="active">' . esc_html__( 'Page', 'zylo' ) . ' ' . get_query_var( 'paged' ) . '</li>';
        }
    }
endif;
